...
package object fs2dsl {
  ...
  /**
   * Creates a stream of message bodies consumed from the Camel endpoint identified by `uri`.
   * Message bodies are converted to type `O` using a Camel type converter. The stream completes
