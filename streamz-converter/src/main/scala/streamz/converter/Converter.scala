...
trait Converter {
   ...
   */
  def akkaSourceToFs2Stream[F[_], A, M](source: Graph[SourceShape[A], M])(implicit materializer: Materializer, F: Async[F]): Stream[F, A] =
    Stream.force {
      F.delay {
   ...
   */
  def akkaSinkToFs2Pipe[F[_], A, M](sink: Graph[SinkShape[A], M])(implicit materializer: Materializer, F: Async[F]): Pipe[F, A, Unit] = { s =>
    Stream.force {
      F.delay {
...
trait ConverterDsl extends Converter {
  ...
  implicit class AkkaSinkMatDsl[A, M](sink: Graph[SinkShape[A], Future[M]]) {
    def toPipeMat[F[_]: Async](implicit materializer: Materializer): Pipe[F, A, Either[Throwable, M]] =
