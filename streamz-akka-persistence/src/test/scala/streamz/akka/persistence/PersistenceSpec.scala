...
class PersistenceSpec extends TestKit(ActorSystem("test")) with WordSpecLike with Matchers with BeforeAndAfterAll {
  ...
  "A replayer" must {
    "produce a discrete stream of journaled messages" in {
      ...
      replay("p1").take(3).runLog.unsafeRun should be(Seq(Event("p1", 1L, "1"), Event("p1", 2L, "2"), Event("p1", 3L, "3")))
    }
    "produce a discrete stream of journaled messages from user-defined sequence number" in {
      ...
      1 to 3 foreach { i => p ! i.toString }
      replay("p2", 2L).take(2).runLog.unsafeRun should be(Seq(Event("p2", 2L, "2"), Event("p2", 3L, "3")))
    }
  }

  "A journal" must {
    "journal a stream of messages" in {
      Stream("a", "b", "c").journal("p3").run.unsafeRun
      replay("p3").map(p => (p.data, p.sequenceNr)).take(3).runLog.unsafeRun should be(Seq(("a", 1L), ("b", 2L), ("c", 3L)))
    }
  }
  ...
  "A snapshot loader" must {
    "produce the most recent snapshot" in {
      ...
      snapshot[String]("p4").runLog.unsafeRun should be(Seq(Snapshot(metadata, "ab")))
    }
    "produce a zero snapshot if there's no snapshot stored" in {
      snapshot[String]("p5").runLog.unsafeRun should be(Seq(Snapshot(SnapshotMetadata("p5", 0L, 0L), "")))
    }
  }
  ...
  "A composition of snapshot and replay" must {
    "produce a discrete stream of updated states" in {
      ...
      c.take(3).runLog.unsafeRun should be(Seq("ab", "abc", "abcd"))
    }
  }
}
