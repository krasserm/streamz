...
public class CamelAkkaJavaDslExample implements JavaDsl {
    private ActorMaterializer actorMaterializer;
    private StreamContext streamContext;

    public CamelAkkaJavaDslExample() throws Exception {
        ActorSystem actorSystem = ActorSystem.create("example");
        SimpleRegistry camelRegistry = new SimpleRegistry();
        DefaultCamelContext camelContext = new DefaultCamelContext();

        camelRegistry.put("exampleService", new CamelAkkaJavaDslExampleService());
    ...
    public static void main(String... args) throws Exception {
        new CamelAkkaJavaDslExample().setup().run();
    }
}
